...
export function CloseConnection() {
  ...
  const handleCloseConnections = async () => {
    ...
    try {
      ...
      // Fecha o diálogo após 2 segundos em caso de sucesso
      setTimeout(() => {
        setIsOpen(false);
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="ext-px-4 ext-py-2 ext-bg-orange-500 ext-border ext-border-orange-300 ext-rounded-md ext-text-sm ext-font-medium ext-text-white hover:ext-bg-orange-200"
      >
        {t('pages.connections.close.button')}
      </button>
    );
  }
        ...
        <div className="ext-flex ext-min-h-full ext-items-center ext-justify-center ext-p-4">
          ...
          >
            <div className="ext-p-6">
              <h3 className="ext-text-lg ext-font-medium ext-text-gray-900">{t('pages.connections.close.title')}</h3>
