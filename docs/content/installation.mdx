...
<Tabs items={["Chrome Web Store", "Manual Installation", "Development Build"]}>
  <Tabs.Tab>
    **Extension Not Appearing** - Check if installation completed successfully -
    Look for error messages in the store - Try refreshing the Chrome Web Store
    page **Installation Fails** - Ensure you have a stable internet connection -
    Check if Chrome is up to date - Try installing from a different network
  </Tabs.Tab>
  <Tabs.Tab>
    **"Invalid manifest" Error** - Ensure you selected the correct folder -
    Check if manifest.json exists and is valid - Verify the folder structure is
    correct **Extension Not Loading** - Check Chrome's developer console for
    errors - Ensure all required files are present - Try reloading the extension
  </Tabs.Tab>
  <Tabs.Tab>
    **Build Errors** - Check Node.js version (requires 18+) - Ensure pnpm is
    installed globally - Clear node_modules and reinstall **Extension Not
    Working** - Check if RabbitMQ is running - Verify the extension is loaded in
    Chrome - Check browser console for errors
  </Tabs.Tab>
</Tabs>
